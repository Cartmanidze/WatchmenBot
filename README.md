...
| `/ask <вопрос>` | Дерзкий ответ про людей в чате на основе истории сообщений |
| `/smart <вопрос>` | Серьёзный вопрос с поиском в интернете (Perplexity) |
...
┌─────────────────────────────────────────────────────────────────┐
│  1. ПОИСК КОНТЕКСТА                                             │
│     SearchSimilarAsync(вопрос, limit=20)                        │
│     → 20 самых релевантных сообщений из embeddings              │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│                                                                 │
│     Stage 1: ФАКТЫ (temp=0.3)                                   │
│     Prompt: "Кто связан с темой? Что говорили? Цитаты?"         │
│     → Точные факты из контекста                                 │
│                                                                 │
│     Stage 2: ПОДЪЁБКА (temp=0.6)                                │
│     Prompt: "Ответь дерзко, подколи того кто связан с темой"    │
│     → Едкий юмор с матом                                        │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  РЕЗУЛЬТАТ: Короткий дерзкий ответ (2-3 предложения)            │
└─────────────────────────────────────────────────────────────────┘
```

**LLM**: `uncensored` тег → Grok (без цензуры)
**Temperature**: 0.3 → 0.6
**Требует**: наличие контекста в embeddings

---

### `/smart` — Серьёзный вопрос

```
┌─────────────────────────────────────────────────────────────────┐
│  1. ПОИСК КОНТЕКСТА (опционально)                               │
│     SearchSimilarAsync(вопрос, limit=20)                        │
│     → Контекст из чата (если есть)                              │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  2. ГЕНЕРАЦИЯ ОТВЕТА                                            │
│     LLM с поиском в интернете                                   │
│     Prompt: контекст + вопрос + дата                            │
│     → Ответ с источниками                                       │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
**Temperature**: 0.5
**Работает**: с контекстом чата или без него
...
| `/ask` | контекст (20) | Grok | uncensored | 0.3→0.6 | 2 |
| `/smart` | контекст (опц.) | Perplexity | factcheck | 0.5 | 1 |
| `/search` | поиск | — | — | — | 0 |
