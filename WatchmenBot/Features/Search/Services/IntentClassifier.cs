    ...
    ILogger<IntentClassifier> logger)
{
    private const string SystemPrompt = """
        ...
        - personal_self: вопрос о ПОЛЬЗОВАТЕЛЕ который спрашивает ("я", "какой я", "обо мне", "что я сказал")
          ВАЖНО: НЕ включает вопросы О БОТЕ! "ты создан", "зачем ты", "ты разочарован" — это factual, не personal_self!
        - personal_other: вопрос о конкретном человеке из чата ("что за тип Глеб", "@vasia как?", "кто такой X")
        - factual: общий вопрос о теме ИЛИ вопрос о боте ("что такое X", "зачем ты создан", "ты умеешь Y")
