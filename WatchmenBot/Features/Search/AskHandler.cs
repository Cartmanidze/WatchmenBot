...
public class AskHandler
{
        ...
        string question, string context, string? memoryContext, string askerName, PromptSettings settings, DebugReport debugReport, CancellationToken ct)
    {
        ...
        var humorPrompt = $"""
            ...
            Вопрос: {question}
            {memorySection}
            Извлечённые факты из чата:
            {factsResponse.Content}

            ПРАВИЛА:
            1. Используй ТОЛЬКО факты выше — они уже отфильтрованы по релевантности
            2. НЕ придумывай новых фактов или цитат
            3. Отвечай СТРОГО на вопрос "{question}"
            4. Если есть "roast_target" — подколи этого человека
            5. Если есть "best_quote" — вставь цитату дословно в <i>кавычках</i>
            6. Ответ должен быть дерзким и с матом
            7. Если facts пустой — честно скажи что ничего не нашёл по теме
            8. НИКОГДА не упоминай технические детали: JSON, формат данных, структуру — просто отвечай
            9. Имена пиши ТОЧНО как в данных (Gleb Bezrukov, НЕ "Глеб Безухов"!)

            ПАМЯТЬ О ПОЛЬЗОВАТЕЛЕ:
            - Используй ТОЛЬКО если релевантна текущему вопросу
            - НЕ упоминай факты из памяти, если они НЕ связаны с вопросом
            - Если вопрос про баню — не тяни сюда бильярд!
            - Подкол должен быть ПО ТЕМЕ вопроса, а не случайным
