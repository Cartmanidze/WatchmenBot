...
public class PromptSettingsStore(IDbConnectionFactory connectionFactory, ILogger<PromptSettingsStore> logger)
{
    ...
    private static readonly Dictionary<string, PromptConfig> DefaultPrompts = new()
    {
        ...
        // ============================================================
        // RECALL COMMAND - Remember discussion topic
        // ============================================================

        ["recall:funny:ru"] = new PromptConfig
        {
            Description = "Вспоминает что обсуждалось по теме (весёлый режим)",
            LlmTag = null,
            SystemPrompt = """
                ...
                - Можешь добавить лёгкую подколку
                - Если информации мало — честно скажи
                - Формат: HTML (<b>, <i>), без markdown
                """
        },

        ["recall:business:ru"] = new PromptConfig
        {
            Description = "Вспоминает что обсуждалось по теме (деловой режим)",
            LlmTag = null,
            SystemPrompt = """
                Ты помогаешь вспомнить, что обсуждалось в чате по заданной теме.
