...
- **Debug отчёт `/ask`** — полностью переработан для лучшей понятности:
  - **Секция "Уверенность в ответе"**:
    - Человекочитаемые объяснения для каждого уровня (High/Medium/Low/None)
    - Подробные пояснения метрик (Best Score, Gap, FullText) с интерпретацией
  - **Улучшенная визуализация результатов поиска**:
    - Качественные метки для каждого результата (🌟 отлично, 👍 хорошо, 🤔 средне, 😐 слабо)
    - Отображение времени сообщения с относительной датой (2ч назад, 3д назад)
    - Понятные причины исключения из контекста с пояснениями
  - **Новая секция "ВЕРДИКТ"**:
    - Итоговая оценка качества ответа (ОТЛИЧНОЕ/ХОРОШЕЕ/СРЕДНЕЕ/НИЗКОЕ)
    - Многофакторный анализ (confidence, score, context size, news dumps, gap)
    - Рекомендации по улучшению поиска если результаты слабые

### Removed

- **Entity variations в RAG Fusion** — удалена логика генерации вариаций с именами участников чата для "who is X" вопросов:
  - **Проблема**: Для запроса "кто за хохлов топит?" генерировались бессмысленные вариации ["Женя топит", "топит Женя"] которые засоряли поиск нерелевантными результатами
  - **Что удалено** из `RagFusionService`:
    - `IsWhoQuestion()` — определение "who" вопросов
    - `ExtractEntityWord()` — извлечение "entity" (ошибочно извлекало глаголы как характеристики)
    - `GenerateEntityVariations()` — генерация "[имя] [entity]" паттернов
    - `ApplyEntityBoost()` — буст результатов с entity+name
    - Параметр `participantNames` из `SearchWithFusionAsync()`
  - **Что удалено** из `SearchStrategyService`:
...
- **NicknameExtractionService quality improvements** — улучшено качество извлечения никнеймов из сообщений:
  - **Расширен ExcludedWords** — добавлено ~100 новых слов:
    - Мат и экспрессивная лексика ("блять", "пиздец", "хуй", etc.)
    - Распространённые глаголы ("смотри", "слушай", "погоди", "держи", etc.)
    - Наречия и прилагательные ("много", "мало", "хорошо", "плохо", etc.)
    - Приветствия и вежливые слова ("спасибо", "пожалуйста", "извини", etc.)
  - **Валидация глагольных окончаний** — отклоняются слова с типичными окончаниями глаголов ("-ть", "-ться", "-ите", "-йте", etc.)
  - **Валидация наречий** — отклоняются слова с окончаниями "-но", "-ло", "-ко", "-во"
  - **Результат**: значительное снижение "мусорных" никнеймов вроде "Много", "Смотрите", "блять"
...
- **Perplexity Fallback for Low-Confidence Queries** — когда поиск по чату ничего не нашёл, автоматически переключаемся на Perplexity:
  - **Проблема**: при `Confidence: None` (similarity < 0.25) бот отправлял "🤷 В истории чата про это не нашёл" и не вызывал LLM
  - **Решение**: вместо раннего выхода продолжаем с `context = null` и роутим на Perplexity (тег "factcheck")
  - **Изменения**:
    - `ConfidenceGateService`: вместо `shouldContinue = false` возвращает `true` с warning "🔍 В чате не нашёл, ищу в интернете..."
    - `AnswerGeneratorService`: при `/ask` без контекста переключается на настройки `/smart` (Perplexity)
  - **Результат**: бот всегда даёт ответ — из чата или из интернета

- **Message Truncation in Context Windows** — ограничение длины сообщений в контексте для предотвращения исчерпания бюджета:
  - **Проблема**: одно длинное сообщение (описание стикера ~2000 символов) съедало весь бюджет контекста, вытесняя релевантные результаты
  - **Решение**:
    - Обычные сообщения в окне: max 400 символов
    - Центральное сообщение (найденное поиском): max 800 символов
    - Стикеры: удаляются ссылки на файлы `(stickers/*.tgs)`, остаётся только эмодзи/описание
  - **Результат**: в контекст помещается больше релевантных сообщений вместо одного гигантского

- **Detailed Context Builder Logging** — детальное логирование построения контекста для диагностики (уровень Information):
  - **Top 10 кандидатов** — показывает все результаты поиска с id, similarity, типом (preformatted/expanded)
  - **Expansion tracking** — сколько сообщений запрошено vs сколько реально расширено
  - **Fallback detection** — ⚠️ Warning когда сообщение НЕ найдено в `messages` таблице
  - **Budget tracking** — пошаговое отслеживание бюджета символов для каждого диалога (✅/❌)
  - **Summary** — итоговая статистика: включённые id, исключённые по бюджету, % использования бюджета
  - **Помогает диагностировать**:
    - Почему сообщение найдено в поиске, но не попало в контекст LLM
    - Какие сообщения используют fallback (возможно устаревшие embeddings)
    - Как распределяется бюджет между диалогами
...
- **Exact match not working for short words** — точное совпадение не работало для коротких слов:
  ...
  - **Дополнительно** — не использовался стемминг для морфологических вариантов
  - **Решение** — `ExtractIlikeWords` теперь:
    - Включает слова >= 3 символов (вместо > 3)
...
- **Enhanced CLAUDE.md with LSP Workflow** — обновлена документация по работе с кодом:
  ...
  - Примеры workflow для типичных задач (рефакторинг, поиск мёртвого кода)
...
- **User Guide Documentation** — полное руководство пользователя на русском языке (`docs/USER_GUIDE.md`):
  ...
  - Написано простым языком для обычных пользователей

- **Forwarded Messages Handling (Обработка пересланных сообщений)** — правильная обработка репостов и пересылок:
  - **Сохранение источника** — тип (user/channel/chat/hidden_user), имя автора, дата оригинала
  - **Исключение из профиля** — пересланные сообщения не анализируются как мысли пользователя
  - **Форматирование в эмбеддингах** — `🔄 Вася переслал от канала «Новости»: [текст]`
  - **Отображение в контексте** — в результатах поиска видно, что это репост
  - Миграция БД: новые колонки `is_forwarded`, `forward_origin_type`, `forward_from_name`, `forward_from_id`, `forward_date`

- **Gender Detection (Определение пола)** — гибридное определение пола пользователя для персонализации ответов:
  - **По имени** — словарь из ~200 русских мужских и ~150 женских имён (включая уменьшительные)
  - **По сообщениям** — анализ грамматических форм глаголов (был/была, сказал/сказала)
  - **Гибридный подход** — комбинирует оба метода с учётом confidence score
  - Автоматическое определение при первом взаимодействии с ботом
  - Хранение в БД с confidence для отслеживания качества
  - Использование в контексте памяти для правильного использования местоимений
